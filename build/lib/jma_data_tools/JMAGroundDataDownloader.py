...
class JMAGroundDataDownloader:
    def __init__(self, amedas_file='Amedas_list.csv', output_path='grounddata_download'):
        ...
        self.downloaded_data = None  # Stores the last downloaded data as a DataFrame
        self.data_url = None  # Stores the URL used to download the last data set
    ...
    def download_amedas(self, point, date, dtype='hourly'):
        """
        Download and process AMEDAS ground data for a specific station, date, and data type.
        
        Parameters:
            point (str): Station ID for the AMEDAS location.
            date (pd.Timestamp): Date for data retrieval.
            dtype (str): Data type ('hourly' or 'daily').
        
        Returns:
            None: The downloaded data and URL are saved as instance attributes.
        """
        ...
        try:
            ...
            if table != None:
                ...
                if dtype == 'daily':
                    index = [ pd.to_datetime( year + '-'+month+'-01') + pd.Timedelta( str(int(h)-1)+' day')  for h in do0.loc[:,'日'].values ]
                ...
                self.downloaded_data = do 
                self.output_file = ofile
                self.data_url =  contenturl
...
if __name__ == "__main__":
    ...
    for date in date_range[-1:]:
        ...
        df, url = downloader.download_amedas(point, date, dtype='daily')
